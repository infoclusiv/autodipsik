    ...
    const steps = [
      {
        ...
        expected: "Required selectors, file path, file payload, and prompt text should be present.",
        run: async function runStep(context) {
          context.fileExtension = getFileExtension(workflowInput.filePath, workflowInput.filePayload && workflowInput.filePayload.name);
          context.promptLength = workflowInput.promptText.length;
          const missingSelectors = contracts.requiredSelectorsForMainWorkflow.filter(function missingRequired(key) {
          ...
          if (!workflowInput.filePath) {
            throw buildWorkflowError("FILE_PATH_REQUIRED", "File path is required.", {
              profile: profile,
              expected: "A local Excel file path should be provided.",
              actual: "No file path was supplied."
            });
          }
        ...
        run: async function runStep(context) {
          ...
          if (!chatInputMatch) {
            throw buildWorkflowError("CHAT_INPUT_NOT_FOUND", "The DeepSeek chat input could not be found.", {
              ...
              selector: profile.selectors.chatInput,
              nextChecks: [
                "Confirm you are logged into DeepSeek and the composer is visible.",
                "Verify the stored chat input selectors against the current page."
              ]
            });
          }
